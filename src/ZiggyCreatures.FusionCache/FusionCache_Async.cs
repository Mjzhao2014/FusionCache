...
public partial class FusionCache
{
	...
	public async ValueTask<TValue> GetOrSetAsync<TValue>(string key, Func<FusionCacheFactoryExecutionContext<TValue>, CancellationToken, Task<TValue>> factory, MaybeValue<TValue> failSafeDefaultValue = default, FusionCacheEntryOptions? options = null, IEnumerable<string>? tags = null, CancellationToken token = default)
	{
		...
		try
		{
			...
			if (_logger?.IsEnabled(LogLevel.Information) ?? false)
				_logger.Log(LogLevel.Information, "FUSION [N={CacheName} I={CacheInstanceId}] (O={CacheOperationId} K={CacheKey}): GetOrSetAsync<T> return {Entry}", CacheName, InstanceId, operationId, key, entry.ToLogString(_configurationManager.Options.IncludeTagsInLogs));
	...
	public async ValueTask<TValue?> GetOrDefaultAsync<TValue>(string key, TValue? defaultValue = default, FusionCacheEntryOptions? options = null, CancellationToken token = default)
	{
		...
		try
		{
			...
			if (_logger?.IsEnabled(LogLevel.Information) ?? false)
				_logger.Log(LogLevel.Information, "FUSION [N={CacheName} I={CacheInstanceId}] (O={CacheOperationId} K={CacheKey}): GetOrDefaultAsync<T> return {Entry}", CacheName, InstanceId, operationId, key, entry.ToLogString(_configurationManager.Options.IncludeTagsInLogs));
		...
		where TEntry : class, IFusionCacheEntry
	{
		...
		if (CanExecuteRawClear() == false)
		{
			...
			{
				var _tmp = await GetOrSetAsync<long>(ClearRemoveTagCacheKey, FusionCacheInternalUtils.SharedTagExpirationDataFactoryAsync, 0L, _configurationManager.TagsDefaultEntryOptions, FusionCacheInternalUtils.NoTags, token).ConfigureAwait(false);
			...
			if (entryTimestamp <= ClearRemoveTimestamp)
			{
				...
				await RemoveInternalAsync(key, _configurationManager.CascadeRemoveByTagEntryOptions, token).ConfigureAwait(false);
		...
		if (tags is not null && tags.Length > 0)
		{
			...
			{
				var tagExpiration = await GetOrSetAsync<long>(GetTagCacheKey(tag), FusionCacheInternalUtils.SharedTagExpirationDataFactoryAsync, 0L, _configurationManager.TagsDefaultEntryOptions, FusionCacheInternalUtils.NoTags, token).ConfigureAwait(false);
				if (entryTimestamp <= tagExpiration)
				{
					...
					if (_logger?.IsEnabled(LogLevel.Trace) ?? false)
						...
						await ExpireInternalAsync(key, _configurationManager.CascadeRemoveByTagEntryOptions, token).ConfigureAwait(false);
		...
		if (ClearExpireTimestamp < 0 || (HasDistributedCache && HasBackplane == false))
		{
			...
			{
				var _tmp = await GetOrSetAsync<long>(ClearExpireTagCacheKey, FusionCacheInternalUtils.SharedTagExpirationDataFactoryAsync, 0L, _configurationManager.TagsDefaultEntryOptions, FusionCacheInternalUtils.NoTags, token).ConfigureAwait(false);
	...
	internal async ValueTask<(bool error, bool isSame, bool hasUpdated)> TryUpdateMemoryEntryFromDistributedEntryAsync<TValue>(string operationId, string key, FusionCacheMemoryEntry<TValue> memoryEntry)
	{
		...
		try
		{
			...
			try
			{
				var (distributedEntry, isValid) = await dca.TryGetEntryAsync<TValue>(operationId, key, _configurationManager.TryUpdateEntryOptions, false, Timeout.InfiniteTimeSpan, default).ConfigureAwait(false);
