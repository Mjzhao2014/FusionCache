...
public partial class FusionCache
{
	...
	private IFusionCacheMemoryEntry? GetOrSetEntryInternal<TValue>(string operationId, string key, string[]? tags, Func<FusionCacheFactoryExecutionContext<TValue>, CancellationToken, TValue> factory, bool isRealFactory, MaybeValue<TValue> failSafeDefaultValue, FusionCacheEntryOptions? options, Activity? activity, CancellationToken token)
	{
		...
		if (memoryEntryIsValid)
		{
			...
			MaybeRenewEntrySlidingExpiration<TValue>(operationId, key, memoryEntry, options, token);
	...
	/// </summary>
	private void MaybeRenewEntrySlidingExpiration<TValue>(string operationId, string key, IFusionCacheMemoryEntry entry, FusionCacheEntryOptions options, CancellationToken token)
	{
		// Only if sliding expiration configured
		if (options.SlidingExpiration.HasValue == false)
			return;
		// Do not renew if stale (eg fail-safe fallback)
		if (entry.IsStale())
			return;
		// Compute new logical expiration as sliding from now
		...
		{
			// Absolute cap based on original creation
