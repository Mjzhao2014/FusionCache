...
internal partial class BackplaneAccessor
{
	...
	public bool PublishCascadeInvalidateByKey(string operationId, string parentKey, long timestamp, FusionCacheEntryOptions options, bool isAutoRecovery, bool isBackground, CancellationToken token)
	{
		if (_logger?.IsEnabled(LogLevel.Debug) ?? false)
	...
	public bool PublishCascadeInvalidateByTag(string operationId, string parentTag, long timestamp, FusionCacheEntryOptions options, bool isAutoRecovery, bool isBackground, CancellationToken token)
	{
		if (_logger?.IsEnabled(LogLevel.Debug) ?? false)
