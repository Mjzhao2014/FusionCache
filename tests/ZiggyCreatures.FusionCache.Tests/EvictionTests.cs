...
public class EvictionTests
{
	...
	[Fact]
	public async Task FusionCache_LruEviction_NoDuplicateEntriesForSameKey()
	{
		// Arrange
		var evictionEvents = new List<FusionCacheEntryEvictionEventArgs>();
		var config = new FusionCacheEvictionPolicyConfig
		{
			MaxEntryCount = 5,
			EvictionPercentage = 0.4 // Evict 40% when triggered
		};
		...
		using (cache)
		{
		// Act - Fill cache, update same keys multiple times to test duplicate handling
		await cache.SetAsync("key1", "value1_v1");
		await cache.SetAsync("key2", "value2_v1");
		await cache.SetAsync("key3", "value3_v1");
		
		// Update existing keys - should not create duplicates in eviction policy
		await cache.SetAsync("key1", "value1_v2");
		await cache.SetAsync("key2", "value2_v2");
		
		// Add more entries to reach capacity
		await cache.SetAsync("key4", "value4");
		await cache.SetAsync("key5", "value5");
		
		// Access some keys to change LRU order
		await cache.GetOrDefaultAsync<string>("key1");
		await cache.GetOrDefaultAsync<string>("key3");
		
		// Update existing key again
		await cache.SetAsync("key2", "value2_v3");
		
		// Trigger eviction by adding more entries
		await cache.SetAsync("key6", "value6");
		await cache.SetAsync("key7", "value7");
		
		await Task.Delay(100); // Allow eviction to complete

		// Assert - Check that no duplicate keys exist in eviction events
		var evictedKeys = evictionEvents.Select(e => e.Key).ToList();
		var uniqueKeys = evictedKeys.Distinct().ToList();
		
		Assert.True(uniqueKeys.Count == evictedKeys.Count, 
			$"Found duplicate keys in eviction events. Evicted keys: [{string.Join(", ", evictedKeys)}]");
		
		// Verify that each evicted key appears exactly once
		foreach (var key in uniqueKeys)
		{
			var keyCount = evictedKeys.Count(k => k == key);
			Assert.True(keyCount == 1, $"Key '{key}' appears {keyCount} times in eviction events, expected exactly 1");
		}			// Verify all evicted entries are valid
			Assert.All(evictionEvents, e =>
			{
				Assert.NotNull(e.Key);
	...
	[Fact]
	public async Task FusionCache_LfuEviction_NoDuplicateEntriesForSameKey()
	{
		// Arrange
		var evictionEvents = new List<FusionCacheEntryEvictionEventArgs>();
		var config = new FusionCacheEvictionPolicyConfig
		...
		using (cache)
		{
			...
			for (int i = 0; i < 3; i++)
			{
				await cache.GetOrDefaultAsync<string>("key1"); // Make key1 more frequent
			}
			...
			await cache.GetOrDefaultAsync<string>("key2");
			...
			// Update existing key again
			await cache.SetAsync("key3", "value3_v2");
	...
	[Fact]
	public async Task FusionCache_LruEviction_NoDuplicatesUnderConcurrentUpdates()
	{
		// Arrange - Test for duplicates under concurrent access patterns
		var evictionEvents = new List<FusionCacheEntryEvictionEventArgs>();
		var evictionEventsLock = new object();
		var config = new FusionCacheEvictionPolicyConfig
		{
		...
		using (cache)
		{
			// Act - Concurrent updates to same keys and new keys
			var tasks = new List<Task>();
			
			// Add initial entries
			for (int i = 0; i < 5; i++)
			{
				await cache.SetAsync($"key{i}", $"initialValue{i}");
			}
			...
			for (int i = 0; i < 20; i++)
			{
				...
				tasks.Add(Task.Run(async () =>
				{
					var keyIndex = index % 5; // Reuse keys 0-4
					await cache.SetAsync($"key{keyIndex}", $"updatedValue{keyIndex}_{index}");
					await cache.GetOrDefaultAsync<string>($"key{keyIndex}");
				}));
			}
			
			// Add new keys to trigger evictions
			for (int i = 10; i < 25; i++)
			{
				int index = i;
				tasks.Add(Task.Run(async () =>
				{
					await cache.SetAsync($"newkey{index}", $"newValue{index}");
				}));
			}

			await Task.WhenAll(tasks);
			await Task.Delay(200); // Allow eviction to complete
			...
			lock (evictionEventsLock)
			{
				...
				Assert.True(uniqueKeys.Count == evictedKeys.Count, 
					$"Found duplicate keys in eviction events under concurrent access. Evicted keys: [{string.Join(", ", evictedKeys)}]");
	...
	[Fact]
	public async Task FusionCache_LfuEviction_NoDuplicatesUnderConcurrentUpdates()
	{
		// Arrange - Test for duplicates under concurrent access patterns
		var evictionEvents = new List<FusionCacheEntryEvictionEventArgs>();
		var evictionEventsLock = new object();
		...
		using (cache)
		{
			...
			// Add initial entries with different access patterns
			for (int i = 0; i < 5; i++)
			{
				await cache.SetAsync($"key{i}", $"initialValue{i}");
				// Give different frequencies to initial keys
				for (int j = 0; j < i + 1; j++)
				{
