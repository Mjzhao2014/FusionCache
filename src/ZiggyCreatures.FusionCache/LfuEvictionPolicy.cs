...
/// <summary>
/// An in-memory eviction policy that evicts least frequently used items when capacity limits are reached.
/// </summary>
public class LfuEvictionPolicy : IFusionCacheEvictionPolicy
{
	...
	/// <summary>
	/// Create a new LFU eviction policy with the given configuration.
	/// </summary>
	...
	public void OnSet(string key, FusionCacheEntryMetadata? metadata)
	{
		lock (_lock)
		{
			if (_entries.TryGetValue(key, out var info))
			{
				...
				// treat a set as another access (doesn't reset count)
				info.AccessCount++;
			}
			else
			{
	...
	public IEnumerable<string> GetKeysToEvict()
	{
		lock (_lock)
		{
			...
			if (_config.MaxEntryCount.HasValue)
			{
				...
				foreach (var kvp in _entries.OrderBy(e => e.Value.AccessCount).ThenBy(e => e.Value.LastAccessTicks).Take(toEvict))
